...
Hold switch Down until the LEDs change (one second). Release immediately — the chord is written to the next slot. The LED pattern shows which slot number was just stored (LEDs 0, 2, 4 encode the slot in binary; LEDs 1, 3, 5 are full brightness once the one second hold is passed as an indicator a chord has been stored).

There are up to a maximum of eight chords stored.  Storing more than eight will overwrite in a loop.

**Recalling chords**
After that, each rising edge at Pulse In 2 recalls the next chord in sequence, stepping through slots in the order they were stored, wrapping around once the last one stored is recalled. When a chord is held, the LEDs show the held pattern (LEDs 0, 2, 4 full bright; LEDs 1, 3, 5 encode the slot in binary).
...
Each of the 13 Y positions (0–12 semitones) has 6 extension presets to explore. Extensions are chosen to reinforce the harmonic character of the chosen interval — no minor 3rds if you've picked a major 3rd, no 3rds at all if you've picked a 5th or unison.

| Y | Harmonic world | Extensions stay within |
|---|---------------|------------------------|
| 0 (unison) | Open/power | 5ths and octaves only |
| 1 (min 2nd) | Cluster/ambiguous | Semitone stacks, tritones, m7ths — no 3rds |
| 2 (maj 2nd) | Sus2 | 2nds, 4ths, 5ths, 9ths — no 3rds |
| 3 (min 3rd) | Minor | m3, P5, m7 — no M3 |
| 4 (maj 3rd) | Major | M3, P5, M7, 9th — no m3 |
| 5 (perf 4th) | Sus4/quartal | 4ths, 5ths, octaves — no 3rds |
| 6 (tritone) | Diminished/augmented | Tritones, m3, M6 — no P5 |
| 7 (perf 5th) | Power chord | 5ths, octaves, 9th — no 3rds |
| 8 (min 6th) | Minor/Phrygian | m3, P5, m6, m7 — no M3 or M6 |
| 9 (maj 6th) | Major/pentatonic | M3, P5, M6 — no minor colours |
| 10 (min 7th) | Dominant/blues | P5, m7, 9th — no M7 |
| 11 (maj 7th) | Lydian/maj7 | M3, P5, M7, #4 — no m7 |
| 12 (octave) | Quartal/suspended | 4ths, 5ths, 2nds — no 3rds |
...
3. Switch to Mid, Main Knob slightly CW for gentle 5-cent detune
...
- 6-phase-accumulator oscillators
